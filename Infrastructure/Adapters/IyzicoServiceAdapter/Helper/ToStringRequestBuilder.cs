...
public class ToStringRequestBuilder
{
    ...
    public ToStringRequestBuilder AppendList<T>(string key, List<T> list = null) where T : RequestStringConvertible
    {
        ...
        {
            var appendedValue = "";
            foreach (RequestStringConvertible value in list) appendedValue = appendedValue + value.ToPKIRequestString() + ", ";
    ...
    {
        if (!string.IsNullOrEmpty(_requestString)) _requestString = _requestString.Substring(0, _requestString.Length - 1);
        return this;
    }
