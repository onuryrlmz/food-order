...
public class RestaurantTransferService : IRestaurantTransferService
{
    ...
    public async Task TransferDataFromGetir(string getirRestaurantId, Guid restaurantId)
    {
        try
        {
            ...
            if (!File.Exists(productFilePath) || !File.Exists(categoryFilePath)) _getirServiceAdapter.TransferRestaurant(getirRestaurantId);

            if (!File.Exists(productFilePath) || !File.Exists(categoryFilePath)) throw new FileNotFoundException("Getir product or category file not found after transfer attempt.");
    ...
    public async Task TransferDataFromYemekSepeti(string ysRestaurantId, Guid restaurantId)
    {
        try
        {
            ...
            if (!File.Exists(productFilePath) || !File.Exists(categoryFilePath)) throw new FileNotFoundException("YemekSepeti product or category file not found.");
